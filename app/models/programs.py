...
class ProgramCampaign(Base, TimestampMixin, SoftDeleteMixin, LTreePathMixin):
    ...
    alpha_location_id = Column(UUID(as_uuid=False), nullable=True, index=True)
...
class EventAssignment(Base, TimestampMixin, SoftDeleteMixin, LTreePathMixin):
    ...
    assigned_by_id = Column(UUID(as_uuid=True), ForeignKey("users.user_id"), nullable=False, index=True)
    approved_by_id = Column(UUID(as_uuid=True), ForeignKey("users.user_id"), nullable=True, index=True)
